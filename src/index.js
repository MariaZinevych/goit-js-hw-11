...
function searcheImage(e) {
  ...
  fetch(
    `https://pixabay.com/api/?key=19462317-9864fc13c2867a4042c7cafcd&q=${arrInput}&image_type=photo&orientation=horizontal&safesearch=true`
  )
    .then(response => response.json())
      .then(data => { onImage.innerHTML = '';  arrImage(data.hits); }
      )
    .catch(error => {
      if (error) showError();
    });
